...
for (focgene in c("HA", "M1", "NA", "NP", "NS1", "PA", "PB1", "PB2")){
  ...
  featsets <- list.files(path = "S3\\data\\full\\mlready", pattern = focgene)
  ...
  for (i in 1:length(featsets)) {
    ...
    # Load in data
    train <- inner_join(read.csv("S3\\data\\full\\cluster_rep_labels.csv") %>% 
                          select(cluster_rep, label),
                        readRDS(paste0("S3\\data\\full\\mlready\\",featsets[i])) %>% 
