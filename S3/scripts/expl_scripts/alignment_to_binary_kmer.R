...
binary_conversion <- function(sequences, k, output_folder) {
  ...
  foreach(kmer = all_kmers) %dopar% {
    # Only run if not already done, or if already done and failed (small file)
    if (!file.exists(file.path(output_folder, paste0("binary_", kmer, ".csv"))) |
        (file.exists(file.path(output_folder, paste0("binary_", kmer, ".csv"))) &
        file.size(file.path(output_folder, paste0("binary_", kmer, ".csv"))) < 50000)) {
