...
result <- foreach (subtypepicked = c("H4N8","H5N1")) %do% {
  ...
  model_list <- model_list[names(model_list) %in% (coefs %>% filter(s1 != 0) %>% pull(param))]
  
  set.seed(1146)
  plr_stack <- caretStack(model_list %>% as.caretList(),
                          method = "glmnet",      # Can change as needed
                          #lower.limits=rep(0, length(model_files)), # Constrain to positive coefficients
