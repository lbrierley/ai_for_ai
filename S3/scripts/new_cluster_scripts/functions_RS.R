...
process_GISAID_seq <- function(x, label, type){
  ...
  if (type == "prot"){
    ...
    # Manual fix for if title itself contains vertical bars
    df %<>%
      mutate(
        UID = case_when(gene == "Protein_INSDC" ~ str_extract(fastahead, "([^|]+)(?=(?:\\|[^|]*){6}$)") ,
                        TRUE ~ UID),
        subtype = case_when(gene == "Protein_INSDC" ~ str_extract(fastahead, "([^|]+)(?=(?:\\|[^|]*){5}$)") %>% str_sub(.,  5, -1) ,
                            TRUE ~ subtype),
        date = case_when(gene == "Protein_INSDC" ~ str_extract(fastahead, "([^|]+)(?=(?:\\|[^|]*){3}$)") ,
                         TRUE ~ date),
        protINSDC = case_when(gene == "Protein_INSDC" ~ str_extract(fastahead, "([^|]+)(?=(?:\\|[^|]*){2}$)") ,
                              TRUE ~ protINSDC),
        protaccession = case_when(gene == "Protein_INSDC" ~ str_extract(fastahead, "([^|]+)(?=(?:\\|[^|]*){1}$)") ,
                                  TRUE ~ protaccession),
        gene = case_when(gene == "Protein_INSDC" ~ str_extract(fastahead, "[^|]+$"),
                         TRUE ~ gene),
        gid = UID
      ) 
  }
...
#process_segmented_sequences is a function that calls process_GISAID_seq and process_NCBI_Seq on nucleotide level-data, seperately for all 8 segments, and then binds them together
#The output is two dataframes, one for avian sequences and one for human sequences. 
process_segmented_sequences <- function(path, ending, type_label) {
  
  if (!dir.exists(path)) {
    stop(paste("Folder does not exist:", path))
  }
  
  ending_regex <- paste0("(?i)", ending)
  files <- list.files(
    path,
    full.names = TRUE,
    pattern = paste0("(?i)", ending, ".*\\.(fa|fasta)$")
  )
  
  if (length(files) == 0) {
    stop(paste("No files found in", path, "matching ending:", ending))
  }
  ...
  file_info <- tibble(
    ...
    mutate(
      source = str_extract(filename, "(?i)GISAID|NCBI"),
      host   = str_extract(filename, "(?i)Avian|Human")
    )
  
  read_group <- function(df, source_name, host_name) {
    group_files <- df %>%
      filter(str_to_upper(source) == str_to_upper(source_name),
             str_to_upper(host)   == str_to_upper(host_name)) %>%
      pull(file)
    
    if (length(group_files) == 0) return(NULL)
    
    seqs <- map(group_files, Biostrings::readDNAStringSet) #Read DNA strings for nucleotide data 
    seqs <- do.call(c, seqs)
    
    return(seqs)
  }
  
  GISAID_avian  <- read_group(file_info, "GISAID", "Avian")
  GISAID_human  <- read_group(file_info, "GISAID", "Human")
  NCBI_avian    <- read_group(file_info, "NCBI",   "Avian")
  NCBI_human    <- read_group(file_info, "NCBI",   "Human")
  
  out <- list(
    GISAID_avian_df = if (!is.null(GISAID_avian))
      process_GISAID_seq(GISAID_avian, "nz",   type_label) else NULL,
    
    GISAID_human_df = if (!is.null(GISAID_human))
      process_GISAID_seq(GISAID_human, "zoon", type_label) else NULL,
    
    NCBI_avian_df   = if (!is.null(NCBI_avian))
      process_NCBI_seq(NCBI_avian, "nz",   type_label) else NULL,
    
    NCBI_human_df   = if (!is.null(NCBI_human))
      process_NCBI_seq(NCBI_human, "zoon", type_label) else NULL,
    
    raw_GISAID_avian = GISAID_avian,
    raw_GISAID_human = GISAID_human,
    raw_NCBI_avian   = NCBI_avian,
    raw_NCBI_human   = NCBI_human
  )
  
  return(out)
}

#process_segmented_sequences_pro is a function that calls process_GISAID_seq and process_NCBI_Seq on amino acid level-data, seperately for all 8 segments, and then binds them together
#The output is two dataframes, one for avian sequences and one for human sequences. 
#Uses ReadAAStringSet instead of ReadDNAStringSet to allow it to recognise protein data. 
process_segmented_sequences_pro <- function(path, ending, type_label) {
  
  internal_type <- ifelse(tolower(type_label) == "cds", "cds", type_label)
  ...
  read_group <- function(df, source_name, host_name) {
    ...
    seqs <- map(group_files, Biostrings::readAAStringSet) #ReadAAStringSet - allows this function to be used on protein data
    seqs <- do.call(c, seqs) # concatenating
  ...
  out <- list(
    GISAID_avian_df = if (!is.null(GISAID_avian))
      process_GISAID_seq(GISAID_avian, "nz",   internal_type) else NULL,
    
    GISAID_human_df = if (!is.null(GISAID_human))
      process_GISAID_seq(GISAID_human, "zoon", internal_type) else NULL,
    
    NCBI_avian_df   = if (!is.null(NCBI_avian))
      process_NCBI_seq(NCBI_avian, "nz",   internal_type) else NULL,
    
    NCBI_human_df   = if (!is.null(NCBI_human))
      process_NCBI_seq(NCBI_human, "zoon", internal_type) else NULL,
...
#build_metadata_excels combines all metadata excel files within a defined folder into two metadata files, one combining all avian data and one combining all human data.
#The function also removes any duplicate rows created by downloading the metadata segment-by-segment. 
build_metadata_excels <- function(metadata_dir) {
  ...
  if (length(files) == 0) {
    stop("No .xls metadata files found in: ", metadata_dir)
  }
  ...
  read_xls_with_meta <- function(file, host) {
    df <- read_xls(file, .name_repair = "minimal")
  ...
  meta_all <- pmap_dfr(
    list(file_info$file,file_info$host),
    read_xls_with_meta
  )
  
  meta_all <- meta_all %>%
    mutate(Isolate_Id = str_trim(as.character(Isolate_Id))) %>%
    distinct(Isolate_Id, .keep_all = TRUE)
  
  meta_human <- meta_all %>% filter(host == "Human")
  meta_avian <- meta_all %>% filter(host == "Avian")
  
  writexl::write_xlsx(
    list(Metadata = meta_human),
    file.path(metadata_dir, "GISAID_Human_Metadata.xlsx")
  )
  
  writexl::write_xlsx(
    list(Metadata = meta_avian),
    file.path(metadata_dir, "GISAID_Avian_Metadata.xlsx")
  )
  
  message("Created metadata files with duplicates removed using Isolate_Id.")
...
#bind_ORF is a function that finds the longest open reading frame (ORF) within a nucleotide sequence. 
#All reading frames are checked, and an ORF is defined as the longest sequence that starts with a start codon (ATG) and ends with a stop codon (TAA, TAG or TGA). 
bind_ORF <- function(seq) {
  seq <- toupper(seq)
  n <- nchar(seq)
  ...
  if (best_len == 0) {
    return(data.frame(start = 0, end = 0))
  }
  
  data.frame(start = best_start, end = best_end)
}

#Helper functions for extracting sequences and gids from the output of the protein load in. 
#raw_to_lookup function extracts the strings from the raw protein data and adds it to the dataframe
raw_to_lookup <- function(raw_seq) {
  if (is.null(raw_seq)) return(NULL)
  
  tibble(
    fastahead = gsub(" ", "_", names(raw_seq)),
    raw_string = as.character(raw_seq, use.names = FALSE)
  )
}

#extract_NCBI_accession function fixes issue where the accesion code was not calculated properly affecting the gid creation
#This function extracts the NCBI accession code from FASTAs created by NCBI data where accession code is found in the third | | bracket partition
extract_ncbi_accession <- function(raw_seq) {
  if (is.null(raw_seq)) return(NULL)
  
  tibble(
    fastahead = gsub(" ", "_", names(raw_seq)),
    raw_accession = str_extract(
      names(raw_seq),
      "(?<=\\|)[A-Z]{1,3}_?[0-9]+(?:\\.[0-9]+)?(?=\\|?$)"
    )
  )
...
# write_gene_fastas function writes one FASTA per gene for a single dataframe
write_gene_fastas <- function(df, df_name, output_dir) {
  if (is.null(df) || nrow(df) == 0) return(invisible(NULL))
  
  base_name <- str_remove(df_name, "_df$") #removing _df from fasta name
  ...
  genes <- sort(unique(df$gene)) #sorting by gene
  
  for (g in genes) {
    sub_df <- df %>%
      filter(gene == g) %>%
      distinct(gid, string, .keep_all = TRUE)
    
    if (nrow(sub_df) == 0) next
    
    seqs <- Biostrings::AAStringSet(sub_df$string) #setting up string to be fasta sequence 
    names(seqs) <- sub_df$gid # setting gid as fasta heading 
    
    out_file <- file.path(output_dir, paste0(base_name, "_", g, ".fasta")) # setting name for fasta file
    Biostrings::writeXStringSet(seqs, filepath = out_file, format = "fasta") #writing fasta file 
  }
...
calc_composition_counts <- function(x, codonpairs = FALSE){
  ...
  cds_id <- x %>% names()
  codontable <- x %>% codonTable
  enc <- codontable %>% ENC(stop.rm = FALSE) # Calculate Effective Number of Codons (including STOP codons)
  GC_content <- x %>% letterFrequency("GC", as.prob = TRUE) * 100 %>% as.vector() # Calculate % GC content
  nuc_counts <- x %>% letterFrequency(c("A", "C", "G", "T")) # Nucleotide counts
  dinuc_counts <- x %>% dinucleotideFrequency() # Dinucleotide counts
  
  # Nucleotide, dinucleotide counts starting position 1 only
  p1_nuc <- oligonucleotideFrequency(x, width = 1, step = 3)
  p1_dinuc  <- dinucleotideFrequency(x, step = 3)
  
  # Nucleotide, dinucleotide counts starting position 2 only
  x_2 <- x %>% DNAStringSet(start = 2)
  p2_nuc <- oligonucleotideFrequency(x_2, width = 1, step = 3)
  p2_dinuc  <- dinucleotideFrequency(x_2, step = 3)
  
  # Nucleotide, dinucleotide counts starting position 3 only
  x_3 <- x %>% DNAStringSet(start = 3)
  p3_nuc <- oligonucleotideFrequency(x_3, width = 1, step = 3)
  p3_dinuc  <- dinucleotideFrequency(x_3, step = 3)
  
  colnames(p1_nuc) <- paste0(colnames(p1_nuc), "_p1")
  colnames(p2_nuc) <- paste0(colnames(p2_nuc), "_p2")
  colnames(p3_nuc) <- paste0(colnames(p3_nuc), "_p3")
  colnames(p1_dinuc)  <- paste0(colnames(p1_dinuc), "_p1")
  colnames(p2_dinuc)  <- paste0(colnames(p2_dinuc), "_p2")
  colnames(p3_dinuc)  <- paste0(colnames(p3_dinuc), "_p3")
  
  # Codon counts
  codon_counts <- codonCounts(codontable)
  ...
  if(codonpairs == TRUE){
    codonpair_counts <- oligonucleotideFrequency(x, width = 6, step = 3)
    df <- cbind(count_mat, codonpair_counts)
  }
  
  df <- data.frame(cds_id = cds_id,
                   enc = enc,
                   GC_content = GC_content,
                   count_mat) %>%
    rename(GC_content = G.C)
...
calc_kmer_counts <- function(x, k, overlap = TRUE, rescale = TRUE){
  ...
  counts_mat <- DNAStringSet(x$string) %>% oligonucleotideFrequency(k, step=ifelse(overlap == TRUE, 1, k))

  if (rescale == TRUE) {
    mat_row_totals <- rowSums(counts_mat)
    mat_row_totals[mat_row_totals == 0] <- 1 
    counts_mat <- counts_mat / mat_row_totals
  }

  df <- data.frame(gid = x$gid, segment = x$segment, stringsAsFactors = FALSE)
  
  return(cbind(df, as.data.frame(counts_mat)))
...
calc_composition_bias <- function(df, codonpairs = FALSE){
  ...
  df$length = df$A + df$C + df$G + df$T
  
  nuc_p1_names <- col_names[grepl("^[A|C|G|T]_p1$", col_names)]
  nuc_p2_names <- col_names[grepl("^[A|C|G|T]_p2$", col_names)]
  nuc_p3_names <- col_names[grepl("^[A|C|G|T]_p3$", col_names)]
  
  df$n_nucs_p1 <- rowSums(df[, nuc_p1_names, drop = FALSE])
  df$n_nucs_p2 <- rowSums(df[, nuc_p2_names, drop = FALSE])
  df$n_nucs_p3 <- rowSums(df[, nuc_p3_names, drop = FALSE])
  
  # Calculate total dinucleotides (pos 1-2, pos 2-3, pos 3-1) define then calculate
  dinuc_names <- col_names[grepl("^[A|C|G|T][A|C|G|T]$", col_names)]
  dinuc_p1_names <- col_names[grepl("^[A|C|G|T][A|C|G|T]_p1$", col_names)]
  dinuc_p2_names <- col_names[grepl("^[A|C|G|T][A|C|G|T]_p2$", col_names)]
  dinuc_p3_names <- col_names[grepl("^[A|C|G|T][A|C|G|T]_p3$", col_names)]
  
  df$n_dinucs    <- rowSums(df[, dinuc_names, drop = FALSE])
  df$n_dinucs_p1 <- rowSums(df[, dinuc_p1_names, drop = FALSE])
  df$n_dinucs_p2 <- rowSums(df[, dinuc_p2_names, drop = FALSE])
  df$n_dinucs_p3 <- rowSums(df[, dinuc_p3_names, drop = FALSE])
  
  # Calculate total codons
  codon_cols <- col_names[grepl("^[A|C|G|T][A|C|G|T][A|C|G|T]$", col_names)]
  df$n_codons    <- rowSums(df[, codon_cols, drop = FALSE])
  
  # Calculate amino acid frequencies
  
  aa_alphabet <- unique(codon_ref$aminoacid)
  
  for (aa in aa_alphabet) {
    
    codons_for_aa <- codon_ref$codon[codon_ref$aminoacid == aa]
    df[[paste0(aa, "_aa")]] <- rowSums(df[, codons_for_aa, drop = FALSE])
  ...
  for (focal_dinuc in dinuc_names) {
    nuc1 <- substr(focal_dinuc, 1, 1)
    nuc2 <- substr(focal_dinuc, 2, 2)
    df[[paste0(focal_dinuc, "_Bias")]] <- (df[[focal_dinuc]] / df$n_dinucs) / 
      ((df[[nuc1]] / df$length) * (df[[nuc2]] / df$length))
  }
  # Calculate dinucleotide biases separately for positions 1-2, 2-3, 3-1
  for (focal_dinuc in dinuc_p1_names) {
    nuc1 <- substr(focal_dinuc, 1, 1)
    nuc2 <- substr(focal_dinuc, 2, 2)
    df[[paste0(focal_dinuc, "_Bias")]] <- (df[[focal_dinuc]] / df$n_dinucs_p1) / 
      ((df[[paste0(nuc1, "_p1")]] / df$n_nucs_p1) * (df[[paste0(nuc2, "_p2")]] / df$n_nucs_p2))
  }
  
  for (focal_dinuc in dinuc_p2_names) {
    nuc1 <- substr(focal_dinuc, 1, 1)
    nuc2 <- substr(focal_dinuc, 2, 2)
    df[[paste0(focal_dinuc, "_Bias")]] <- (df[[focal_dinuc]] / df$n_dinucs_p2) / 
      ((df[[paste0(nuc1, "_p2")]] / df$n_nucs_p2) * (df[[paste0(nuc2, "_p3")]] / df$n_nucs_p3))
  }
  
  for (focal_dinuc in dinuc_p3_names) {
    nuc1 <- substr(focal_dinuc, 1, 1)
    nuc2 <- substr(focal_dinuc, 2, 2)
    df[[paste0(focal_dinuc, "_Bias")]] <- (df[[focal_dinuc]] / df$n_dinucs_p3) / 
      ((df[[paste0(nuc1, "_p3")]] / df$n_nucs_p3) * (df[[paste0(nuc2, "_p1")]] / df$n_nucs_p1))
  }
  
  # Calculate Relative Synonymous Codon Usage
  codon_list <- codon_ref %>% split(.$codon)
  
  for (focal_codon in codon_cols) {
    
    synonymous_codons <- codon_ref$codon[codon_ref$aminoacid == codon_list[[focal_codon]]$aminoacid]
    
    df[[paste0(focal_codon, "_Bias")]] <- df[[focal_codon]] * codon_list[[focal_codon]]$deg / 
      rowSums(df[, synonymous_codons, drop = FALSE])
    
  }
  
  # Calculate amino acid biases - denominator uses total amino acids, including stop codons, so can just use total codons
  for (aa in aa_alphabet) {
    df[, paste0(aa, "_aa_Bias")] <- df[, paste0(aa, "_aa")]/df$n_codons
  }
