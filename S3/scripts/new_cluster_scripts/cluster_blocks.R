...
# Read in sequences and generate kmer counts at desired k
...
reps_kmer_list <- readRDS("S3\\data\\segmentwise\\clust_dist\\reps_kmer_list.rds")

## Work out pairwise distance metric(s)

# Initialise empty list
pairwise_JSD <- vector(mode = "list", length = 8)

# Calc JSD in parallel
start_time <- Sys.time()
...
hclust_list <- readRDS("S3\\data\\segmentwise\\clust_dist\\hclust_list.rds")
