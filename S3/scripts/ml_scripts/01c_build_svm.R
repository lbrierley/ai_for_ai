...
svm_fun <- function(seg, featset){
  ...
  preds_form <- data.frame(label = train$label, preds_df) # kernlab::svm step only: to specify model as formula rather than x/y
...
foreach (seg = 1:8) %:% 
  foreach (featset = featsets,
           .packages = c("caret","dplyr","kernlab","tidyr","stringr")) %dopar% {
             
             model <- svm_fun(seg, featset)
             
             if (!is.null(model)) {
               # save outputs
               saveRDS(
                 model,
                 file = file.path(out_dir, paste0("svm_", featset, "_pt_", focgene[seg], ".rds"))
               )
             }
